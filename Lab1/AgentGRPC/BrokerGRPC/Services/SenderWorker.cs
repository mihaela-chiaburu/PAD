                ...
                if (message != null)
                {
                    ...
                    foreach (var connection in connections)
                    {
                        ...
                        try
                        {
                            ...
                            Console.WriteLine($"Sent message '{message.Content}' to {connection.Address} - Success: {reply.IsSuccess}");
                        }
                        catch (RpcException rpcException)
                        {
                            if (rpcException.StatusCode == StatusCode.Internal || rpcException.StatusCode == StatusCode.Unavailable)
                            {
                                Console.WriteLine($"gRPC error when sending message to {connection.Address}: Subscriber removed");
                                _connectionStorage.RemoveConnection(connection.Address);
                            }
                            else
                            {
                                Console.WriteLine($"gRPC error when sending message to {connection.Address}: {rpcException.Message}");
                            }
                        }
                        catch (Exception exception)
                        {
                            Console.WriteLine($"Failed to send message to {connection.Address}: {exception.Message}");
                        }
